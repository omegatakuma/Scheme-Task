...
(define (ToDo)
  ...
  (let1 cmd (read)
		(cond ((eq? cmd ':q)(exit))
			  ...
			  ((eq? cmd ':r)(r))
			  ((eq? cmd ':del)(del))
			  (else (begin(print "***ERROR: command not found: "cmd)(ToDo))))))
(define (file-in)
  (with-input-from-file "./.todo" (pa$ read)))
(define (file-out cont)
  (with-output-to-file "./.todo" (pa$ print cont)))
(define (w)
  (let1 words (file-in)
		(if (eof-object? words)
		  (writ (create) '())
						  ...
						  ((eq? n '-a)
						   (begin 
							 ...
							 (let1 ans (read)
								   ...
								   '())))
						  (else (string-append "***ERROR command not found: "n))))(ToDo))))
(provide "taskf/cmd")
